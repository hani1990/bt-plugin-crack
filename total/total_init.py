...
reload (sys )
import json ,os ,time ,public ,string ,web ,re
...
class plugin_total_init:
    ...
    def get_sites(self,get):
        ...
        #if not getattr(web.ctx.session,'bt_total',False):  return modc;
        #result = {}
        #data = []
    ...
    def __get_mod(self,get):
        if os.path.exists('/www/server/apache'):
            if not os.path.exists('/usr/local/memcached/bin/memcached'): 
                web.ctx.session.bt_total = False
        ...
        if getattr(web.ctx.session,'bt_total',False): return public.returnMsg(True,'OK!');
        ...
        try:
            if not result['status']: 
                if getattr(web.ctx.session,'bt_total',False): del(web.ctx.session['bt_total'])
                return result;
        except: pass;
        web.ctx.session.bt_total = True
        return result
