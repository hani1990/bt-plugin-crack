...
Install_total()
{
	...
	wget -O $pluginPath/total_main.py $download_Url/install/lib/plugin/total/total_main.py -T 5
	wget -O $pluginPath/total_init.py $download_Url/install/lib/plugin/total/total_init.py -T 5
	wget -O $pluginPath/index.html $download_Url/install/lib/plugin/total/index.html -T 5
	wget -O $pluginPath/info.json $download_Url/install/lib/plugin/total/info.json -T 5
	wget -O $pluginPath/icon.png $download_Url/install/lib/plugin/total/icon.png -T 5
	
	if [ ! -f /www/server/panel/static/js/china.js ];then
		wget -O /www/server/panel/static/js/china.js $download_Url/install/lib/plugin/total/china.js -T 5
	fi
	wget -O /www/server/panel/vhost/apache/total.conf $download_Url/install/lib/plugin/total/total_httpd.conf -T 5
	wget -O /www/server/panel/vhost/nginx/total.conf $download_Url/install/lib/plugin/total/total_nginx.conf -T 5
	\cp -a -r /www/server/panel/plugin/total/icon.png /www/server/panel/static/img/soft_ico/ico-total.png
	wget -O $pluginPath/total.zip $download_Url/install/lib/plugin/total/total.zip -T 5
	...
	if [ ! -f $total_path/config.json ];then
		wget -O $total_path/config.json $download_Url/install/lib/plugin/total/config.json -T 5
	fi
